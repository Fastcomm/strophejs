...
const Strophe = {
  ...
  getText: function(elem) {
    ...
    let str = '';
    if (elem.childNodes.length === 0 && (elem.nodeType === Strophe.ElementType.TEXT || elem.nodeType === Strophe.ElementType.CDATA)) {
      str += elem.nodeValue;
    }
    for (let i = 0; i < elem.childNodes.length; i++) {
      if (elem.childNodes[i].nodeType === Strophe.ElementType.TEXT || elem.childNodes[i].nodeType === Strophe.ElementType.CDATA) {
...
Strophe.Connection.prototype = {
  ...
  _sasl_success_cb: function(elem) {
    ...
    streamfeature_handlers.push(
      this._addSysHandler(elem => wrapper(streamfeature_handlers, elem), Strophe.NS.STREAM, 'features', null, null)
    );
